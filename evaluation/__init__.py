...
import automedts.evaluation.test_evaluator
import automedts.evaluation.train_evaluator
import automedts.evaluation.util
import automedts.pipeline.components
from automedts.automl_common.common.utils.backend import Backend
from automedts.evaluation.train_evaluator import TYPE_ADDITIONAL_INFO
from automedts.metrics import Scorer
from automedts.util.logging_ import PickableLoggerAdapter, get_named_client_logger
...
def fit_predict_try_except_decorator(
    ...
    try:
        ...
        # following traceback:
        #     File "auto-sklearn/automedts/evaluation/__init__.py", line 29, in fit_predict_try_except_decorator  # noqa E501
        #     return ta(queue=queue, **kwargs)
        #     File "auto-sklearn/automedts/evaluation/train_evaluator.py", line 1067, in eval_holdout  # noqa E501
        #     evaluator.fit_predict_and_loss(iterative=iterative)
        #     File "auto-sklearn/automedts/evaluation/train_evaluator.py", line 616, in fit_predict_and_loss,  # noqa E501
        #     status=status
        #     File "auto-sklearn/automedts/evaluation/abstract_evaluator.py", line 320, in finish_up  # noqa E501
        #     self.queue.put(return_value_dict)
        #     File "miniconda/3-4.5.4/envs/automedts/lib/python3.7/multiprocessing/queues.py", line 87, in put  # noqa E501
        #     self._start_thread()
        #     File "miniconda/3-4.5.4/envs/automedts/lib/python3.7/multiprocessing/queues.py", line 170, in _start_thread  # noqa E501
        #     self._thread.start()
        #     File "miniconda/3-4.5.4/envs/automedts/lib/python3.7/threading.py", line 847, in start  # noqa E501
...
def get_cost_of_crash(metrics: Sequence[Scorer]) -> List[float] | float:
    ...
    for metric in metrics:
        ...
        # automedts optimizes the err. This function translates
...
class ExecuteTaFuncWithQueue(AbstractTAFunc):
    def __init__(
        ...
        **resampling_strategy_args: Any,
        ...
        if resampling_strategy == "holdout":
            eval_function = automedts.evaluation.train_evaluator.eval_holdout
        elif resampling_strategy == "holdout-iterative-fit":
            eval_function = (
                automedts.evaluation.train_evaluator.eval_iterative_holdout
            )
        elif resampling_strategy == "cv-iterative-fit":
            eval_function = automedts.evaluation.train_evaluator.eval_iterative_cv
        elif resampling_strategy == "cv" or isinstance(
            resampling_strategy, (BaseCrossValidator, _RepeatedSplits, BaseShuffleSplit)
        ):
            eval_function = automedts.evaluation.train_evaluator.eval_cv
        elif resampling_strategy == "partial-cv":
            eval_function = automedts.evaluation.train_evaluator.eval_partial_cv
        elif resampling_strategy == "partial-cv-iterative-fit":
            eval_function = (
                automedts.evaluation.train_evaluator.eval_partial_cv_iterative
            )
        elif resampling_strategy == "test":
            eval_function = automedts.evaluation.test_evaluator.eval_t
        ...
        self.automedts_seed = automedts_seed
    ...
    def run(
        ...
        obj_kwargs = dict(
            ...
            additional_components=automedts.pipeline.components.base._addons,
        )
        ...
        if obj.exit_status in (
            ...
            try:
                info = automedts.evaluation.util.read_queue(queue)
                result = info[-1]["loss"]
        ...
        if (
            ...
            learning_curve = automedts.evaluation.util.extract_learning_curve(info)
            learning_curve_runtime = automedts.evaluation.util.extract_learning_curve(
                info, "duration"
            )
            ...
            train_learning_curve = automedts.evaluation.util.extract_learning_curve(
                info, "train_loss"
            )
            ...
            if self._get_test_loss:
                test_learning_curve = (
                    automedts.evaluation.util.extract_learning_curve(
                        info,
                        "test_loss",
                    )
                )
        ...
        automedts.evaluation.util.empty_queue(queue)
