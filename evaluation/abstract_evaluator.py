...
import automedts.pipeline.classification
import automedts.pipeline.regression
...
from automedts.data.target_validator import (
    SUPPORTED_TARGET_TYPES,
    SUPPORTED_XDATA_TYPES,
)
from automedts.metrics import Scorer, calculate_losses
from automedts.pipeline.components.base import ThirdPartyComponents, _addons
from automedts.pipeline.implementations.util import (
    convert_multioutput_multiclass_to_multilabel,
)
from automedts.util.logging_ import PicklableClientLogger, get_named_client_logger
    ...
    def __init__(
        ...
        if self.task_type in REGRESSION_TASKS:
            if not isinstance(self.configuration, Configuration):
                ...
                self.model_class = (
                    automedts.pipeline.regression.SimpleRegressionPipeline
                )
            ...
            if not isinstance(self.configuration, Configuration):
                ...
                self.model_class = (
                    automedts.pipeline.classification.SimpleClassificationPipeline
                )
